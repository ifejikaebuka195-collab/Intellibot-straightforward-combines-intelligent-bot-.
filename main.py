...
MAX_PRICES = 5000
TICK_CONFIRMATION = 3
OBSERVE_EXTRA_TICKS = 5  # ✅ ADDED (extra observation)
...
# ================================
# ✅ STABLE TREND FILTER (ADDED ONLY)
# ================================
def stable_trend_only(p):
    if len(p) < 50:
        return False
    ...
    # strict smoothness (low noise)
    if std / mean > 0.004:
        return False

    # consistent direction
    diff = np.diff(p[-15:])
    if np.sum(diff > 0) >= 12 or np.sum(diff < 0) >= 12:
        return True
        ...
        try:
            ...
            async with websockets.connect(DERIV_WS) as ws:
                ...
                async for msg in ws:
                    ...
                    # ✅ ONLY STABLE TREND
                    if not stable_trend_only(prices[pair]):
                        continue
                    ...
                    # ✅ EXTRA OBSERVATION (ADDED ONLY)
                    await asyncio.sleep(OBSERVE_EXTRA_TICKS)
